...
# ===========================================================================
# [*] FAST DEEP LEARNING CACHE SYSTEM
# ===========================================================================
import time
import pickle
from sklearn.metrics.pairwise import cosine_similarity

# Global cache for ultra-fast deep learning inference
FAST_DL_CACHE = {
    'ready': False,
    'model': None,
    'user_embeddings': None,        # (943, 128) numpy array
    'movie_embeddings': None,       # (1682, 128) numpy array  
    'user_similarities': None,      # (943, 943) pre-computed
    'user_id_to_idx': {},          # {user_id: matrix_index}
    'movie_id_to_idx': {},         # {movie_id: matrix_index}
    'idx_to_user_id': {},          # {matrix_index: user_id}
    'idx_to_movie_id': {},         # {matrix_index: movie_id}
    'last_trained': None,
    'cache_version': 1.0
}

def is_fast_dl_ready():
    """Check if fast deep learning cache is ready"""
    return FAST_DL_CACHE.get('ready', False)

def get_cache_status():
    """Get detailed cache status"""
    if not FAST_DL_CACHE['ready']:
        return {
            'status': 'not_ready',
            'message': 'Deep learning cache is initializing...'
        }
    
    return {
        'status': 'ready',
        'users_cached': len(FAST_DL_CACHE.get('user_id_to_idx', {})),
        'movies_cached': len(FAST_DL_CACHE.get('movie_id_to_idx', {})),
        'last_trained': FAST_DL_CACHE.get('last_trained'),
        'cache_version': FAST_DL_CACHE.get('cache_version')
    }

def update_user_embedding_incremental(user_id: int, movie_id: int, new_rating: float):
    """Update single user's embedding with new rating - ultra fast"""
    try:
        if not is_fast_dl_ready():
            return False
        
        # Check if user and movie exist in cache
        if user_id not in FAST_DL_CACHE['user_id_to_idx']:
            logger.warning(f"[!] User {user_id} not in cache")
            return False
        
        if movie_id not in FAST_DL_CACHE['movie_id_to_idx']:
            logger.warning(f"[!] Movie {movie_id} not in cache") 
            return False
        
        # Get indices
        user_idx = FAST_DL_CACHE['user_id_to_idx'][user_id]
        movie_idx = FAST_DL_CACHE['movie_id_to_idx'][movie_id]
        
        # Get current user embedding
        current_embedding = FAST_DL_CACHE['user_embeddings'][user_idx].copy()
        
        # Get movie embedding (if available from model)
        if hasattr(main_deep_learning_system, 'movie_embeddings'):
            movie_embedding = main_deep_learning_system.movie_embeddings[movie_idx]
            
            # Calculate rating impact
            rating_impact = (new_rating - 3.0) / 2.0  # Normalize to [-1, +1]
            learning_rate = 0.05  # Conservative update
            
            # Update embedding
            current_embedding += learning_rate * rating_impact * movie_embedding
            
            # Normalize to maintain vector magnitude
            current_embedding = current_embedding / np.linalg.norm(current_embedding)
            
            # Update cache
            FAST_DL_CACHE['user_embeddings'][user_idx] = current_embedding
            
            # Update similarity row for this user only
            update_user_similarity_row(user_idx)
            
            logger.info(f"[⚡] User {user_id} embedding updated incrementally")
            return True
        
        return False
        
    except Exception as e:
        logger.error(f"[x] Incremental update error: {e}")
        return False

def update_user_similarity_row(user_idx: int):
    """Update similarity scores for single user - very fast"""
    try:
        user_embedding = FAST_DL_CACHE['user_embeddings'][user_idx]
        all_embeddings = FAST_DL_CACHE['user_embeddings']
        
        # Calculate similarities with all other users
        similarities = cosine_similarity([user_embedding], all_embeddings)[0]
        
        # Update only this user's row in similarity matrix
        FAST_DL_CACHE['user_similarities'][user_idx] = similarities
        
        # Also update this user's column (symmetric matrix)
        FAST_DL_CACHE['user_similarities'][:, user_idx] = similarities
        
        logger.debug(f"[⚡] Similarity row updated for user index {user_idx}")
        
    except Exception as e:
        logger.error(f"[x] Similarity update error: {e}")

print("[+] Fast Deep Learning Cache System initialized")
...
# ===========================================================================
# [*] FAST DEEP LEARNING STARTUP TRAINING
# ===========================================================================

@app.on_event("startup")
async def initialize_fast_deep_learning():
    """Background model training at startup - user won't wait"""
    logger.info("[*] Starting Fast Deep Learning initialization...")
    
    # Start background training (non-blocking)
    import asyncio
    asyncio.create_task(background_model_training())
    
    logger.info("[+] Fast Deep Learning initialization started in background")

async def background_model_training():
    """Train model in background and prepare cache"""
    try:
        logger.info("[🧠] Background: Training neural model...")
        start_time = time.time()
        
        # Use existing clean_dynamic_deep_recommender
        if main_deep_learning_system is None:
            logger.error("[x] Main deep learning system not available")
            return
        
        # Train model (this will take 30 seconds)
        training_success = main_deep_learning_system.train_model()
        
        if not training_success:
            logger.error("[x] Background training failed")
            return
        
        # Extract embeddings from trained model
        logger.info("[📊] Extracting embeddings for cache...")
        
        # Get user and movie mappings from the trained system
        user_mappings = main_deep_learning_system.user_id_to_idx
        movie_mappings = main_deep_learning_system.movie_id_to_idx
        
        # Get embeddings from the model
        user_embeddings = main_deep_learning_system.user_embeddings
        
        # Create reverse mappings
        idx_to_user = {idx: user_id for user_id, idx in user_mappings.items()}
        idx_to_movie = {idx: movie_id for movie_id, idx in movie_mappings.items()}
        
        # Pre-compute similarity matrix for ultra-fast lookup
        logger.info("[⚡] Pre-computing similarity matrix...")
        similarity_matrix = cosine_similarity(user_embeddings)
        
        # Update global cache
        global FAST_DL_CACHE
        FAST_DL_CACHE.update({
            'ready': True,
            'model': main_deep_learning_system,
            'user_embeddings': user_embeddings,
            'user_similarities': similarity_matrix,
            'user_id_to_idx': user_mappings,
            'movie_id_to_idx': movie_mappings,
            'idx_to_user_id': idx_to_user,
            'idx_to_movie_id': idx_to_movie,
        ...
        cache_data = {
            ...
            'cache_version': FAST_DL_CACHE['cache_version']
        }
        
        with open('fast_dl_cache.pkl', 'wb') as f:
            pickle.dump(cache_data, f)
        
        elapsed = time.time() - start_time
        logger.info(f"[+] 🚀 FAST DEEP LEARNING READY! Training took {elapsed:.1f}s")
        logger.info(f"[+] Cache: {len(user_mappings)} users, similarity matrix ready")
        
    except Exception as e:
        logger.error(f"[x] Background training error: {e}")
        FAST_DL_CACHE['ready'] = False

# Cache status endpoint
@app.get("/fast-dl-status")
async def get_fast_dl_status():
    """Check if fast deep learning system is ready"""
...
async def rate_movie(
    ...
    try:
        ...
        if is_fast_dl_ready():
            try:
                cache_updated = update_user_embedding_incremental(user_id, movie_id, rating)
                if cache_updated:
                    logger.info(f"[⚡] Fast cache updated for user {user_id}")
            except Exception as cache_error:
                logger.warning(f"[!] Cache update failed (continuing): {cache_error}")
        ...
        return {
            ...
            "fast_cache_updated": cache_updated
        }
...
# ===========================================================================
# [*] ⚡ ULTRA-FAST DEEP LEARNING ENDPOINT 
# ===========================================================================

@app.get("/fast-deep-recommendations/{user_id}")
async def get_ultra_fast_deep_recommendations(
    user_id: int,
    n_recommendations: int = 10,
    current_user: dict = Depends(get_current_user)
):
    """⚡ Ultra-fast deep learning recommendations - inference only, no training"""
    
    start_time = time.time()
    
    # Check if cache is ready
    if not is_fast_dl_ready():
        # Fallback to popular movies while cache is building
        db = SessionLocal()
        try:
            popular_movies = db.query(Movie).filter(
            ...
            for movie in popular_movies:
                fallback_recs.append({
                    ...
                    "reason": "Popular movie (cache initializing)",
                    "algorithm": "fallback_while_training"
                })
            
            return {
                "status": "initializing",
                "message": "🔄 Deep learning cache initializing, showing popular movies",
                "method": "Fallback - Popular Movies", 
                "cache_status": get_cache_status(),
                "recommendations": fallback_recs,
                "note": "Fast recommendations will be available in ~30 seconds"
            }
        finally:
            db.close()
    
    try:
        # ULTRA-FAST INFERENCE (No Training!)
        
        # Step 1: Get user index from cache (0.001s)
        if user_id not in FAST_DL_CACHE['user_id_to_idx']:
            raise HTTPException(status_code=404, detail="User not found in trained model")
        
        user_idx = FAST_DL_CACHE['user_id_to_idx'][user_id]
        
        # Step 2: Get recent ratings to update embedding (0.05s)
        db = SessionLocal()
        try:
            recent_ratings_query = """
                SELECT movie_id, JSON_EXTRACT(extra_data, '$.rating') as rating, timestamp 
                FROM user_interactions 
                WHERE user_id = :user_id AND interaction_type = 'rating'
                AND timestamp > :timestamp
                ORDER BY timestamp DESC
            """
            
            cache_time = FAST_DL_CACHE.get('last_trained', datetime.now() - timedelta(hours=1))
            recent_ratings = db.execute(text(recent_ratings_query), {
                "user_id": user_id,
        ...
        # Step 3: Get base embedding and apply recent updates (0.1s) 
        base_embedding = FAST_DL_CACHE['user_embeddings'][user_idx].copy()
        current_embedding = base_embedding
        
        # Apply incremental updates from recent ratings
        updates_applied = 0
        for movie_id, rating_str, timestamp in recent_ratings:
            if movie_id in FAST_DL_CACHE['movie_id_to_idx']:
                rating = float(rating_str)
                movie_idx = FAST_DL_CACHE['movie_id_to_idx'][movie_id]
                
                # Get movie embedding (if available)
                if hasattr(main_deep_learning_system, 'movie_embeddings'):
                    movie_embedding = main_deep_learning_system.movie_embeddings[movie_idx]
                    
                    # Apply incremental update
                    rating_impact = (rating - 3.0) / 2.0  # [-1, +1]
                    learning_rate = 0.05
                    current_embedding += learning_rate * rating_impact * movie_embedding
                    updates_applied += 1
        ...
        if updates_applied > 0:
            current_embedding = current_embedding / np.linalg.norm(current_embedding)
            logger.info(f"[⚡] Applied {updates_applied} recent rating updates to user {user_id}")
        
        # Step 4: Find similar users using updated embedding (0.05s)
        similarities = cosine_similarity([current_embedding], FAST_DL_CACHE['user_embeddings'])[0]
        
        # Get top similar users (excluding self)
        similar_indices = np.argsort(similarities)[::-1]
        top_similar_indices = [idx for idx in similar_indices if idx != user_idx][:10]
        
        similar_users = []
        for idx in top_similar_indices:
            similar_user_id = FAST_DL_CACHE['idx_to_user_id'][idx]
            similar_users.append({
                'user_id': int(similar_user_id),  # Convert numpy.int64 to int
                'similarity_score': float(similarities[idx])  # Convert numpy.float64 to float
            })
        
        # Step 5: Generate recommendations from similar users (0.1s)
        recommendations = main_deep_learning_system.get_recommendations(
            user_id, n_recommendations
        )
        
        elapsed = time.time() - start_time
        logger.info(f"[⚡] Fast inference completed in {elapsed:.3f}s")
        
        return {
            "status": "success",
            "message": f"⚡ FAST NEURAL - {updates_applied} güncel güncelleme, {len(similar_users)} benzer kullanıcı",
            "method": "Ultra-Fast Neural Inference",
            "algorithm": "cached_embeddings_with_incremental_updates",
            "inference_time_seconds": round(elapsed, 3),
            "cache_hits": True,
            "recent_updates_applied": updates_applied,
            "similar_users_found": len(similar_users),
            "similar_users": similar_users,
            "embedding_dimension": 128,
            "recommendations": recommendations,
            "quality": "fast_cached_deep_learning"
        }
        
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"[⚡] Fast inference error: {e}")
        raise HTTPException(status_code=500, detail=f"Fast deep learning failed: {str(e)}")
