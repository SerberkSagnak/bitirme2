...
# OLD PROBLEMATIC ENDPOINT REMOVED - CLEAN VERSION BELOW
        
# # ADIM 1: KALITE KONTROL
# db = SessionLocal()
        
        # User'ın rating'lerini al
        user_ratings = db.execute(text("""
            SELECT ui.movie_id, 
        ...
        # Tür çeşitliliği kontrol
        all_genres = set()
        for rating in user_ratings:
            if rating[2]:  # genres column
                genres = rating[2].split('|')
                all_genres.update(genres)
        
        genre_count = len(all_genres)
        
        logger.info(f"[📊] Quality Check - User {user_id}: {rating_count} ratings, {genre_count} genres")
            
        # QUALITY GATE KONTROLÜ - COMMENT FIXED
        # MIN_RATINGS = 10
        # MIN_GENRES = 3
        # 
        # # Yetersiz rating kontrolü  
        # if rating_count < MIN_RATINGS:
        #         return {
                    "status": "insufficient_data",
                    "message": f"🎬 DERİN ÖĞRENME İÇİN DAHA FAZLA FİLM PUANLAYIN! ({rating_count}/{MIN_RATINGS})",
                    "method": "Quality Gate - Insufficient Ratings",
                    ...
                    "next_steps": [
                        f"{MIN_RATINGS - rating_count} daha film puanlayın",
                        "Farklı türlerden filmler seçin",
                        "Action, Comedy, Drama gibi çeşitli türler deneyin"
                    ],
                    "recommendations": []
                }
